...
namespace WorldOfTank.Class.Components
{
    ...
    public static class MathProcessor
    {
        ...
        {
            if (CounterClockWise(pA, pB, pC) * CounterClockWise(pA, pB, pD) != -1)
            {
                return false;
            }
            if (CounterClockWise(pC, pD, pA) * CounterClockWise(pC, pD, pB) != -1)
            {
                return false;
            }
            return true;
        ...
        {
            var rad = (float)Math.PI * angle / 180;
        ...
        {
            var difference = calculatedAngle - referenceAngle;
